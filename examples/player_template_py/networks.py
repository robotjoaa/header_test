...
class Actor(nn.Module):
    def __init__(self, nb_states, nb_actions, init_w=3e-3):
        ...
        for i in range(0, self.num_layers):
            self.middle_linear.append(torch.nn.Linear(self.hidden_dim, self.hidden_dim))
    ...
    def init_weights(self, init_w):
        ...
        for i in range(0, self.num_layers):
            self.middle_linear[i].weight.data = fanin_init(self.middle_linear[i].weight.data.size())
